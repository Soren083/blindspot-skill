...
它不是凭空写出来的。它来自真实产品开发中整理出的 **376 个 Codex、Claude Code 和 PieBox 纠错瞬间**，背后对应本机 **300 个 Codex session body**、一批包含 **377 个本地 session 标识**的 Claude Code 历史记录，以及 PieBox 的本地 session、message、diff 和 LLM dump 存储。保留下来的证据包括完整 transcript、子代理轨迹、桌面端会话索引、prompt 历史、usage 事件、message parts 和 session diff。

这些错误往往不是语法错误，而是更危险的那种：代码能跑、方案听起来合理、UI 看起来也更干净，但一到真实用户使用、真机测试、审核环境或线上发布，就会暴露问题。
